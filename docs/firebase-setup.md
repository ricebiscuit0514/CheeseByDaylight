...
- 방장 탭이나 브라우저가 닫히면 참가자는 최대 5분간 재접속을 기다린 뒤
  자동으로 로컬 점수판으로 돌아가며 Firebase 연결 슬롯을 반환합니다.
- 점수판의 로컬 `localStorage` 복구 시간은 기존과 동일하게 1시간입니다.
...
점수 거부, 방장 이탈 후 5분 접근 종료를 실제 Realtime Database Emulator에서
확인합니다.
